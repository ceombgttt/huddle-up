...
The Main Sponsor is the most premium placement - a fixed orange/amber banner at the very top of all pages (above navigation) for logged-in users. Shows sponsor logo, name, tagline, and "MAIN SPONSOR" label. Only one Main Sponsor at a time across the entire platform. When no real main sponsor is active, a demo example (Victory Sports Drink) is shown with an "Advertise" button. The MainSponsorBanner component is defined at the top level and rendered via fixed positioning (z-60). All screen containers have pt-10 and sticky headers use top-10 to account for the banner height. Demo logo: public/demo-sponsors/victory-sports-main.png.
