...
- **Soft Launch Banner**: Prominent gradient banner on home page announcing Boca Raton soft launch. Shows real-time stats (users, parties, venues). First 100 members get lifetime Pro + "Founding Member" badge. Dismiss persists 7 days via localStorage. DB column: `users.is_founder`. Endpoints: `GET /api/users/soft-launch-stats`, `POST /api/users/claim-founder`.
