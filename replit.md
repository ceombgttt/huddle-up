...
- **Bottom Navigation Bar**: Fixed bottom nav on all logged-in screens with 4 tabs: Home, Browse, My Parties, Profile. Active tab highlighted in #1E90FF blue. z-[50], bg-[#151A22]. All screen containers have pb-[72px] to prevent content overlap.
- **Simplified Hamburger Menu**: Restructured with user profile section at top, 4 primary items (Browse, My Parties, Favorite Teams, Notifications), 3 secondary items (Settings, Help & FAQ, More Options), collapsible "More Options" submenu (Install App, Team Chat, Find Fans, Predictions, Fantasy, Invite Friends, Learn The App), plus Venue Hub/Admin/Logout.
...
- **Home Screen Quick Actions**: Location badge with city name and "Change" button, enlarged "Find Watch Parties" CTA with dynamic party count, and 3 quick action cards (My Teams, Upcoming, Near Me) in a grid row.
